...
# SÜRÜ AYARLARI
# ==========================================
NODE_ID = 1               # LUTFEN BURAYI KENDI DRONUNUZA GORE DEGISTIRIN (1, 2, 3, 4)
TOTAL_NODES = 4
SLOT_DURATION = 1.0
...
# ==========================================
# HC-12 KESKİN NİŞANCI AYARLARI
# ==========================================
SDR_FREQ = 433.0e6          # SDR Merkez frekans
TARGET_FREQ = 433.4e6       # HC-12'nin TAM frekansi
SDR_SAMPLE_RATE = 2.048e6
SDR_TARGET_GAIN_DB = 25.0   # HC-12 zayif guc yaydigi icin gain yuksek tutuldu
...
class LocalizationNode:
    ...
    def get_filtered_rssi(self):
        ...
        try:
            for _ in range(3):
                ...
                # HC-12 Filtresi +-20kHz (GFSK Yayin)
                target_offset_hz = TARGET_FREQ - SDR_FREQ
                window_width_hz = 20e3 
    ...
    def run(self):
        logging.info(f"TDMA Basladi. Dongu: {CYCLE_DURATION}s")
        last_tx_cycle = -1
        while self.running:
            now = time.time()
            ...
            if sleep_time > 0.05: time.sleep(sleep_time - 0.02)
            while time.time() < target_time: time.sleep(0.002)
