...
# --- TEST MODU: Pervaneler TAKILI DEĞİLKEN, sadece MAVLink paketlerinin
# doğru gidip gitmediğini doğrulamak için ---
# True yapıldığında SADECE irtifa kontrolü ("havada değil") atlanır; diğer
# tüm güvenlik kontrolleri (heartbeat kaybı, armed olma, GUIDED modu) HÂLÂ
# ÇALIŞIR. Bu, gerçek uçuşta drone'ların irtifa doğrulaması yapılmadan komut
# alması riskini taşır - o yüzden aşağıda program başlarken büyük bir uyarı
# basılır. GERÇEK UÇUŞTAN ÖNCE MUTLAKA False'A GERİ ALIN.
SKIP_AIRBORNE_CHECK_FOR_TESTING = True
...
class DroneLink:
    ...
    def is_ready_for_commands(self):
        with self.lock:
            ...
            # Test modunda SADECE bu irtifa kontrolü atlanır - pervanesiz
            # test sırasında drone gerçekten yükselemeyeceği için bu kontrol
            # normal şartlarda hep "havada değil" döndürür.
            if not SKIP_AIRBORNE_CHECK_FOR_TESTING:
...
class GroundStation:
    def __init__(self, lora_port, pixhawk_ports, simulation, platform_label=""):
        ...
        if SKIP_AIRBORNE_CHECK_FOR_TESTING:
            print("\n" + "!" * 60)
            print("!!  DİKKAT: SKIP_AIRBORNE_CHECK_FOR_TESTING = True     !!")
            print("!!  İrtifa güvenlik kontrolü DEVRE DIŞI.                !!")
            print("!!  Sadece pervanesiz masaüstü/paket testi için        !!")
            print("!!  kullanın. GERÇEK UÇUŞTAN ÖNCE bu bayrağı           !!")
            print("!!  ground_station_core.py içinde False YAPIN.         !!")
            print("!" * 60 + "\n")
    ...
    def _handle_line(self, line):
        ...
        try:
            ...
            #
            # DÜZELTME (bug): bu blok daha önce self.lock / self.latest_rssi
            # diye VAR OLMAYAN bir kilit/sözlüğe yazıyordu (sadece
            # self.rssi_lock / self.rssi_data tanımlı). Bu yüzden her Drone 3
            # paketinde AttributeError fırlıyor, dışarıdaki genel except
            # tarafından yutuluyor ve LoRa gereksiz yere yeniden bağlanıyordu.
            # SONUÇ: Drone 3'ün verisi self.rssi_data'ya HİÇBİR ZAMAN
            # girmiyordu, bu yüzden calculate_virtual_center_shift() her
            # zaman "4/4 taze veri yok" görüp shift'i (0,0) döndürüyordu -
            # yani hedef fiziksel olarak hareket etse bile sanal merkez asla
            # kaymıyordu. Artık normal yoldaki AYNI kilidi/sözlüğü kullanıyor.
