...
# ==========================================
# DÜZELTME 4: SANAL MERKEZ KONTROLÜ - BİRİKTİRİCİDEN (INTEGRATOR) YAKINSAYAN
# KONTROLE (CONVERGING CONTROL) GEÇİŞ
# ==========================================
# ESKİ DAVRANIŞ (bug): her döngüde `virtual_center += shift` yapılıyordu -
# yani RSSI dengesizliği sıfıra inmediği sürece (ki masaüstü/pervanesiz
# testte drone fiziksel olarak hareket etmediği için RSSI ASLA sıfıra
# inmez) sanal merkez HER SANİYE biraz daha büyümeye devam ediyor, çok
# hızlı biçimde ±20m geofence sınırına çarpıp orada RSSI gürültüsü
# yüzünden titreşiyordu. Bu da "KUZEYE giderken mesafe stabil değil"
# şikayetinin doğrudan sebebiydi.
#
# YENİ DAVRANIŞ: her döngüde mevcut RSSI dengesizliğinden DOĞRUDAN bir
# "hedef konum" hesaplanır (POSITION_K_GAIN ile), sonra sanal merkez o
# hedefe doğru YUMUŞAKÇA YAKINSAR (EMA/üstel yumuşatma). Sinyal sabitse
# sanal merkez de birkaç döngü içinde SABİT bir değerde durur - sonsuza
# kadar büyümez. Sinyal gerçekten değişirse (hedef hareket ederse) yeni
# dengeye yine yumuşakça kayar. Bu, gerçek uçuşta da çalışmaya devam eder
# (drone fiziksel olarak yaklaştıkça RSSI dengesizliği zaten küçülür).
POSITION_K_GAIN = 0.5          # dB başına metre (hedef ofseti hesaplarken)
POSITION_SMOOTH_ALPHA = 0.3    # 0-1: büyükse daha hızlı ama daha oynak,
                                # küçükse daha yavaş ama daha stabil
...
class GroundStation:
    def __init__(self, lora_port, pixhawk_ports, simulation, platform_label=""):
        ...
        print(f"RSSI_MAX_AGE_S={RSSI_MAX_AGE_S}s | "
              ...
              f"POSITION_K_GAIN={POSITION_K_GAIN} | POSITION_SMOOTH_ALPHA={POSITION_SMOOTH_ALPHA}")

        if BENCH_TEST_MODE:
            print("\n" + "!" * 60)
            print("!!  DİKKAT: BENCH_TEST_MODE = True                      !!")
            print("!!  İrtifa güvenlik kontrolü DEVRE DIŞI.                !!")
            print("!!  GERÇEK UÇUŞTAN ÖNCE bu bayrağı False YAPIN.         !!")
            print("!" * 60 + "\n")
    ...
    # KARAR MOTORU
    # -------------------------------------------------
    def calculate_target_offset(self, rssi_data):
        """
        Mevcut (yumuşatılmış) RSSI dengesizliğinden DOĞRUDAN bir hedef
        konum (target_x, target_y) hesaplar - bu bir "adım" değil, sanal
        merkezin YAKINSAMASI gereken NOKTADIR. send_mavlink_commands()
        içinde bu hedefe doğru yumuşakça (EMA) yaklaşılır.

        Eksen bağımsız tazelik kontrolü: Kuzey-Güney (Drone 1/2) ve
        Doğu-Batı (Drone 3/4) ayrı ayrı değerlendirilir - biri eskiyse
        sadece O eksen mevcut konumunda sabit kalır, diğeri yine tepki
        vermeye devam eder (tek bir drone'un veri kaybı tüm sistemi
        durdurmaz).
        """
        ...
        if not have_ns and not have_ew:
            ...
            print(f"[UYARI] Ne Kuzey-Güney ne de Doğu-Batı çiftinden taze veri var "
                  f"({len(rssi_data)}/4). Eksik Dronelar: {missing}. "
                  f"Sanal merkez son hedefte sabit tutuluyor.")
            return self.virtual_center_x, self.virtual_center_y

        delta_y_dbm = None
        delta_x_dbm = None

        if have_ns:
            delta_y_dbm = rssi_data[1] - rssi_data[2]
            target_y = max(min(delta_y_dbm * POSITION_K_GAIN, MAX_CENTER_DRIFT_M), -MAX_CENTER_DRIFT_M)
        else:
            print("[UYARI] Kuzey-Güney çifti (Drone 1/2) taze değil, bu eksen son hedefte sabit tutuluyor.")
            target_y = self.virtual_center_y

        if have_ew:
            delta_x_dbm = rssi_data[3] - rssi_data[4]
            target_x = max(min(delta_x_dbm * POSITION_K_GAIN, MAX_CENTER_DRIFT_M), -MAX_CENTER_DRIFT_M)
        else:
            print("[UYARI] Doğu-Batı çifti (Drone 3/4) taze değil, bu eksen son hedefte sabit tutuluyor.")
            target_x = self.virtual_center_x

        print(f"[KARAR MOTORU] Sinyal Farkları -> K/G: {delta_y_dbm if delta_y_dbm is not None else 'N/A'}dB, "
              f"D/B: {delta_x_dbm if delta_x_dbm is not None else 'N/A'}dB")
        print(f"[KARAR MOTORU] Hedef Konum -> KUZEYE: {target_y:.1f}m, DOĞUYA: {target_x:.1f}m")
    ...
    # -------------------------------------------------
    def send_mavlink_commands(self, target_x, target_y):
        if self.origin_lat is None or self.origin_lon is None:
            print("[UYARI] Origin henüz ayarlanmadı, komut gönderilmiyor.")
            return

        # Sanal merkezi HEDEFE DOĞRU YUMUŞAKÇA YAKLAŞTIR (EMA) - artık
        # sonsuza kadar büyüyen bir biriktirici (accumulator) DEĞİL. Sinyal
        # sabitse birkaç döngü içinde virtual_center de target'a eşitlenip
        # SABİT kalır; sinyal değişirse yeni hedefe yine yumuşakça kayar.
        a = POSITION_SMOOTH_ALPHA
        self.virtual_center_x = a * target_x + (1 - a) * self.virtual_center_x
        self.virtual_center_y = a * target_y + (1 - a) * self.virtual_center_y

        # Savunma amaçlı: normalde target zaten geofence içinde ama yine de
        # sınırlandırıyoruz.
    ...
    def run(self):
        ...
        while True:
            ...
            print("\n" + "=" * 55)
            target_x, target_y = self.calculate_target_offset(rssi_snapshot)
            self.send_mavlink_commands(target_x, target_y)
            print("=" * 55 + "\n")
